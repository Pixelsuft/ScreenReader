...
from pyaudio import PyAudio as NewPyAudio
from pyaudio import paInt16 as pa_int_16
from pyaudio import paContinue as pyaudio_cont
import cv2
...
def recorder():
    ...
    if record_audio:
        wf = wave_open(filename + '.wav', "wb")
        wf.setnchannels(1)
        p = NewPyAudio()
        wf.setsampwidth(p.get_sample_size(pa_int_16))
        wf.setframerate(44100)
        stream = p.open(
            format=p.get_format_from_width(wf.getsampwidth()), channels=wf.getnchannels(),
            rate=wf.getframerate(), input=True, stream_callback=audio_callback
        )
    while is_recording:
        while not is_paused:
            try:
